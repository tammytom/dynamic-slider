class CarouselView {
  constructor() {
    ...
    this.touchStartX = 0; this.touchStartY = 0; this.touchStartTime = 0; this.touchMoved = false;
    this.tapMaxMove = 10; this.tapMaxTime = 250;

    this.scaleMap = { center:1, side:.9, far:.8 };
  ...
  update(event, data){
    switch(event){
      ...
      case 'openCarousel': 
        ...
        break;
      case 'indexChanged': this.onIndexChanged(data.index, data.card, data.total); break;
      case 'closeCarousel': this.unmountCarousel(); break;
    }
  }
  ...
  onIndexChanged(index, card, total){
    ...
    if (now - this.lastClickTime < this.clickThrottle) {
      ...
      this.syncTimeout = setTimeout(() => {
        if (this.pendingIndex) {
          const p = this.pendingIndex; this.pendingIndex = null;
    ...
    if (this.isTransitioning) { this.pendingIndex = { index, card, total }; return; }
  ...
  positionCards(items, centerIndex){
    ...
    items.forEach((card, i) => {
      card.classList.remove('center','left-adjacent','left-second','right-adjacent','right-second');
      if (i === centerIndex) { card.classList.add('center'); card.style.left = '50%'; }
      else if (i === (centerIndex - 1 + total) % total) { card.classList.add('left-adjacent'); card.style.left = '32.5%'; }
      else if (i === (centerIndex - 2 + total) % total) { card.classList.add('left-second'); card.style.left = '15%'; }
      else if (i === (centerIndex + 1) % total) { card.classList.add('right-adjacent'); card.style.left = '67.5%'; }
      else { card.classList.add('right-second'); card.style.left = '85%'; }
    });
  }

  ensureVisible(){
    this.carouselTrack.querySelectorAll('.carousel-card.positioned').forEach(el => {
  ...
  syncHeights(){
    ...
    const nodes = [...this.carouselTrack.querySelectorAll('.carousel-card.positioned')];
  ...
  updateDesktopDetails(card, index, total){
    if (this.cardCounter) this.cardCounter.textContent = `${index+1}/${total}`;
    const points = (card.details.points || []).slice(0,3);
    this.detailsContent.innerHTML = `
      <h2 class="details-title" id="detailsTitle">${card.details.title}</h2>
      <ul class="details-points">${points.map(p => `<li>${p}</li>`).join('')}</ul>
  ...
  bindDelegatedInteractions(){
    ...
    this.carouselContainer.addEventListener('touchstart', (e) => {
      ...
      this.touchStartX = startX; this.touchStartY = startY; this.touchStartTime = performance.now(); this.touchMoved = false;
    }, { passive: true });
    ...
    this.carouselContainer.addEventListener('touchmove', (e) => {
      ...
      if (Math.abs(e.touches[0].clientX - this.touchStartX) > this.tapMaxMove ||
    ...
    this.carouselContainer.addEventListener('touchend', (e) => {
      if (swiped) {
        ...
        const custom = new CustomEvent('carousel-swipe', { detail:{ dir: dx<0 ? 'next' : 'prev' } });
      ...
      if (!card) return;
      const idx = parseInt(card.dataset.index, 10);
      if (!Number.isNaN(idx) && !this.isTransitioning) this.openMobileModalByIndex(idx);
    }, { passive: true });
  }

  openMobileModalByIndex(idx){
    if (!this.isMobile || this.modalOpen) return;
    const event = new CustomEvent('request-card-data', { detail:{ index:idx } });
    window.dispatchEvent(event);
  }

  updateMobileModal(card, index, total){
    if (this.mobileCounter) this.mobileCounter.textContent = `${index+1}/${total}`;
  ...
  hideMobileModalImmediate(){
    ...
    queueMicrotask(()=>{ this.mobileModal.style.display='none'; this.modalOpen=false; });
  }
